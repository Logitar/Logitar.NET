...
public class PostgresBuildersTests : IAsyncLifetime
{
  private const string ConnectionStringKey = "POSTGRESQLCONNSTR_IntegrationTests";

  private readonly DbConnection _connection;

  public PostgresBuildersTests()
  {
  ...
  {
    ICommand delete = PostgresDeleteBuilder.From(Db.Artists.Table)
      .Where(new OperatorCondition(Db.Artists.Name, PostgresOperators.IsLikeInsensitive("%dream theater%")))
      .Build();
  ...
  public async Task It_should_insert_the_correct_rows()
  {
    ...
    ICommand insert = PostgresInsertBuilder.Into(Db.Reviews.ReviewId, Db.Reviews.AlbumId, Db.Reviews.IsPublished, Db.Reviews.Note, Db.Reviews.Text)
      .Value(review.ReviewId, review.AlbumId, review.IsPublished, review.Note, review.Text)
      .Build();
  ...
  {
    IQuery query = PostgresQueryBuilder.From(Db.Artists.Table)
      .Join(Db.Albums.ArtistId, Db.Artists.ArtistId)
      .Join(Db.Reviews.AlbumId, Db.Albums.AlbumId, new AndCondition(
        new OperatorCondition(Db.Reviews.IsPublished, Operators.IsEqualTo(true)),
        new OperatorCondition(Db.Reviews.Text, Operators.IsNotNull())
      ))
      .WhereOr(
        new OperatorCondition(Db.Artists.Name, PostgresOperators.IsLikeInsensitive("%drE_m%")),
        new OperatorCondition(Db.Artists.Country, Operators.IsLike("Canada")),
        new OperatorCondition(Db.Albums.Title, PostgresOperators.IsLikeInsensitive("%drE_m%"))
      )
  ...
  {
    ICommand update = new PostgresUpdateBuilder()
      .Set(new Update(Db.Reviews.IsPublished, true))
      .Where(new OperatorCondition(Db.Reviews.IsPublished, Operators.IsNotEqualTo(true)))
