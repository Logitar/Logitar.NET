...
public interface IUserFacade
{
  ...
  Task<User?> ReadAsync(string? id = null, string? tenantId = null, string? uniqueName = null,
    string? externalIdentifierKey = null, string? externalIdentifierValue = null, CancellationToken cancellationToken = default);
  Task<CreatedToken?> RecoverPasswordAsync(RecoverPasswordPayload payload, CancellationToken cancellationToken = default);
  Task<User?> RemoveExternalIdentifierAsync(string id, string key, CancellationToken cancellationToken = default);
  ...
  Task<User?> SaveExternalIdentifierAsync(string id, string key, string value, CancellationToken cancellationToken = default);
